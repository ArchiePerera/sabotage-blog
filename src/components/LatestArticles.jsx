  ...
  return (
    <>
      ...
      {arrayArticles.map((a) => (
        ...
        <div key={a.id} className="transition duration-500 ease-in-out transform hover:scale-105 relative flex flex-col mt-6 text-gray-700 bg-white shadow-md bg-clip-border rounded-xl w-full hover:shadow-2xl">
          <div
          ...
          <div className="p-6">
            <h5 className="block mb-2 text-xl font-semibold leading-snug tracking-normal text-blue-gray-900">
              {a.title}{/*  On va afficher le titre de l'article ici  */}
            </h5>
            ...
            <p className="text-sm text-gray-600 mt-4">
              ⏳ Temps de lecture: {Math.ceil(a.content.length / 300)} mn {/* Vous affichez le vrai temps de lecture en bonus */}
            </p>
          </div>
          <div className="flex justify-between items-center p-6 pt-0">
            <div className="flex items-center">
              {/* Vous affichez l'image de l'auteur et le nom en alt */}
              <img src={a.authorImageUrl} alt={a.author} className="w-10 h-10 rounded-full mr-2" />
              <span className="text-sm font-semibold text-gray-900">{a.author}</span>
            </div>
            {/* Vous affichez la date de l'article  au bon format */}
            <span className="text-sm text-gray-600">{new Date(a.date).toLocaleDateString("fr-FR")}</span>
          </div>
