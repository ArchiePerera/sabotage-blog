...
const ArticlePage = () => {
  ...
  useEffect(() =>{
    const selected = articles.find(p => p.id === parseInt(id))
        ...
        <div className="min-h-screen bg-[#0E1217] text-white py-10">
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              <img src={selectedArticle.imageUrl} alt="Article" className="rounded-lg shadow-lg max-w-full h-auto md:w-1/2" /> {/* image de l'article */}
              <div>
                <h2 className="text-2xl text-[#E11D48] mb-2">Résumé</h2>
                <p className="mb-4">{selectedArticle.summary}</p>  {/* Summary de l'article */}
                <div className="flex items-center gap-4 mb-2">
                  {/* Image de l'auteur*/}
                  <img src={selectedArticle.authorImageUrl} alt={""} className="w-10 h-10 rounded-full shadow-lg" />
                  {/* Nom de l'auteur*/}
                  <span>{selectedArticle.author}</span>
                </div>
                {/* Date de l'article à afficher au format français */}
                <div className="text-gray-400 mb-2">📅 {new Date(selectedArticle.date).toLocaleDateString("fr-FR")}</div>
